...
Lambda = complex(imag=1.0) * np.diag([0.0, 1.0, 10.0])
R = np.array([[1, 1, 1]])
rtd = lf.RightTangentialData(Lambda, R, system)


M = complex(imag=1.0) * np.diag([complex(imag=-1.0), 100.0, 1000.0])
L = np.array([  [1],
                [1],
                [1]])
...
bode_plot.add_data_tick(at_frequency=10, pin_to_system_num=0, pin_to_output_num=1, pin_to_input_num=1, markersize=12)
bode_plot.add_data_tick(at_frequency=100, pin_to_system_num=0, pin_to_output_num=1, pin_to_input_num=1, markersize=12)
bode_plot.add_data_tick(at_frequency=1000, pin_to_system_num=0, pin_to_output_num=1, pin_to_input_num=1, markersize=12)
